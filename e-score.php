...
if($_POST){
    ...
    if($status=="1"){
        $sql3="UPDATE team SET status = '1' WHERE team_id = $team_id1";
        if(mysqli_query($link,$sql3)){
            $sql3="UPDATE team SET status = '2' WHERE team_id = $team_id2";
            if(mysqli_query($link,$sql3)){
                $sql3="UPDATE match_team SET results = '2' WHERE match_id = $match_id";
                if(mysqli_query($link,$sql3)){
                    echo "<script>setInterval(function() { location.href = 'e-score.php'; }, 100);
                    alert('เสร็จสิ้น'); </script>";
                }
            }
        }
    }else if($status=="2"){
        $sql3="UPDATE team SET status = '2' WHERE team_id = $team_id1";
        if(mysqli_query($link,$sql3)){
            $sql3="UPDATE team SET status = '1' WHERE team_id = $team_id2";
            if(mysqli_query($link,$sql3)){
                $sql3="UPDATE match_team SET results = '3' WHERE match_id = $match_id";
                if(mysqli_query($link,$sql3)){
                    echo "<script>setInterval(function() { location.href = 'e-score.php'; }, 100);
                    alert('เสร็จสิ้น'); </script>";
                }
        ...
        <tbody>
            <tr>
                <td class="table-dark"><div align="center"><br>
                    <a class="btn btn-secondary" href="store.php">กลับหน้าหลัก</a><br><br>
                </div></td>
                        ...
                        <tr>
                            ...
                            <td class="table-dark"><div align="center">
                                ...
                                <form method="post">
                                    ...
                                    <select name="status" class="form-control" style="width:55%" OnChange="resutName(this.value);">
                                        <?php
                                            include "dblink.php";
